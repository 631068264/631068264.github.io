...
小文件，单层目录小文件（单位 k），文件数也少的可以使用**ConfigMap**
...
spec:
  ...
  - name: mysql
    ...
    volumeMounts: #挂载到容器
      - name: mysql-initdb
        mountPath: /docker-entrypoint-initdb.d #挂载的目录
  volumes:
    - name: mysql-initdb
      configMap:
...
例如nginx前端有大量零碎文件，文件结构复杂的可以把目录**COPY**然后打一个新镜像

```dockerfile
FROM nginx

COPY web/templates /etc/nginx/templates
COPY web/html /etc/nginx/html
```
...
- 应用名
  ...
  定义资源名字时候要使用`/templates/_helpers.tpl` 里面的 `{{ template "fullname" . }}`可以看成是应用名，可能每次不一样
  ...
  */}}
  ...
  metadata:
    labels:
      app: {{ template "fullname" . }}-business
    name: {{ template "fullname" . }}-business
  spec:
    replicas: 1
    selector:
      matchLabels:
  ...
  spec:
    ports:
      - name: "9003"
        port: {{ .Values.business.servicePort }}
    selector:
      app: {{ template "fullname" . }}-business
  ```

- pod之间通信`xxx_host`通过**service name**
  ...
  metadata:
    labels:
      app: {{ template "fullname" . }}-sso
    name: {{ template "fullname" . }}-sso
  spec:
    replicas: 1
    selector:
      matchLabels:
    ...
    template:
      ...
      spec:
        containers:
          - env:
              - name: KAFKA_HOST
                value: {{ template "fullname" . }}-cp-kafka-headless
              - name: MYSQL_HOST
                value: {{ template "fullname" . }}-mysqlha-headless
              - name: NACOS_HOST
                value: {{ .Values.nacos.host }}
              - name: REDIS_HOST
                value: {{ template "fullname" . }}-redis-cluster-headless
              - name: TZ
                value: {{ .Values.TZ }}
            ...
            name: sso
            ports:
              - containerPort: {{ .Values.sso.servicePort }}
            resources: {}
        restartPolicy: Always
  
  ```

- 需要配置的可配置的选项东西最好放到`values.yaml`文件里面

  ```yaml
  # Default values for smartward.
  # This is a YAML-formatted file.
  # Declare variables to be passed into your templates.
  
  # 跨charts自定义
  global:
    mysqlInitDir: /etc/smartward/mysql_init
  ...
  business:
    image: 10.19.64.203:8080/smartward/smartward-business:latest
    servicePort: 9003
  mattress:
    image: 10.19.64.203:8080/smartward/smartward-mattress:latest
    servicePort: 9013
  sso:
    image: 10.19.64.203:8080/smartward/smartward-mattress:latest
    servicePort: 8887
  nginx:
    image: 10.19.64.203:8080/smartward/smartward-nginx:latest
    servicePort: 19000
    nodePort: 31900
