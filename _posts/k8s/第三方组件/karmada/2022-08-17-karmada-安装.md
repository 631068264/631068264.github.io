...
完成后在member2上面看到**derived-**开头的service，可以通过这个svc连接到member1
...
```sh
kubectl --kubeconfig member2-config run -i --rm --restart=Never --image=jeremyot/request:0a40de8 request -- --duration={duration-time} --address={ClusterIP of derived service}
```
