...
- NoSchedule：除非 Pod 明确声明可以容忍这个 Taint, 否则不会被调度到 nodel 上。
- PreferNoSchedule: 一个Pod如果没有声明容 忍这个 Taint, 则系统会尽量避免把这个 Pod 调度到这一节点上，但不是强制的 
